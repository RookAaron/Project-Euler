...
largest_product = 0
grid_size = (20, 20) #20x20

for row_index in range(0,grid_size[0]):
    for coloumn_index in range(0, grid_size[1]):

        # across, down, diagonally down right, diagonally up left
        products = [1, 1, 1, 1]

        for i in range(0,4):
            
            if coloumn_index+4 <= grid_size[1]: # can preform across calc
                products[0] *= grid[row_index][coloumn_index+i]

            if row_index+4 <= grid_size[0]: # can preform down calc
                products[1] *= grid[row_index+i][coloumn_index]

            if coloumn_index+4 <= grid_size[1] and row_index+4 <= grid_size[0]: # can preform diagonal calc
                products[2] *= grid[row_index+i][coloumn_index+i]

            if coloumn_index+4 <= grid_size[1] and row_index-4 >= 0: #
                products[3] *= grid[row_index-i][coloumn_index+i]
                #print(row_index-i, coloumn_index+i)
